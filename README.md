...
nginx listens on **port 80 inside the container**. On the shared droplet: **80/443** = Zyrowaste, **8080** = existing Jenkins, **8090** = timetable app, **8081** = optional dedicated Jenkins for this project.
...
**Environment variables:** `IMAGE_NAME`, `PROD_HOST`, `DEPLOY_PATH`, `APP_PORT` (8090), `JENKINS_HTTP_PORT` (8081), `COMPOSE_PROJECT_NAME` (see Jenkinsfile and `Deploy/droplet.env.example`).
...
**Production URL (v1):** `http://143.244.128.22:8090/` (app). **Dedicated Jenkins:** `http://143.244.128.22:8081/` (existing main Jenkins stays on **8080**).
...
- Firewall allows TCP **8090** (app) and **8081** (timetable Jenkins, if used)
